...
def education_section(education_data,course_data):
    ...
    st.write(f'**Graducation Date:**{education_data["Graduation Date"]}')
...
#activities
def activities_section(leadership_data, activity_data, school_data):
    st.header("Activities")
    tab1, tab2, tab3 = st.tabs(['Leadership','Community Service','School Involvement'])
    with tab1:
    ...
    with tab2:
        ...
        for title, details in activity_data.items():
            ...
            for bullet in details:
                expander.write(bullet)          
    with tab3:
        st.subheader("School Involvement")
        for title, details in school_data.items():
            expander = st.expander(f'{title}')
            for bullet in details:
                expander.write(bullet)
    st.write("---")
    
activities_section(info.leadership_data, info.activity_data, info.school_data)
